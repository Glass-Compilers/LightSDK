...
void getWang(sampler2D s, float2 uv, float tiling, out float2 wangUv, out float4 wangUVForMip)
{
    float2 wangBase = uv * tiling * 4;
    float2 wangUV = wangBase / 128;
    float4 wang = tex2D(s, wangUV);

    wangUVForMip = float4(ddx(wangBase*0.25), ddy(wangBase*0.25));

    wang.ba *= 255.0/256.0;
    wangUv = wang.ba + frac(wangBase)*0.25;
}
