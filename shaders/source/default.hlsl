...
#ifdef PIN_WANG
float4 sampleWangSimple(sampler2D s, float2 uv)
{
    float2 wangUv;
    float4 wangUVForMip;
    getWang(NormalDetailMap, uv, 1, wangUv, wangUVForMip);
    return tex2Dgrad(DiffuseMap, wangUv, wangUVForMip.xy, wangUVForMip.zw);
}
#endif
...
#ifdef PIN_WANG
    #ifdef PIN_WANG_FALLBACK
       float4 albedo = tex2D(DiffuseMap, IN.Uv_EdgeDistance1.xy); 
    #else
	   float4 albedo = sampleWangSimple(DiffuseMap, IN.Uv_EdgeDistance1.xy);
    #endif
#else
